...
void RenderWindow::initilizeGL(float* vertices, int vsize, int* indices, int isize)
{
    timer.init();
...
//    mVBOArray->allocate(vertices, sizeof(vertices));
    mVBOArray->allocate(vertices, vsize);
//    mProgram->enableAttributeArray(0);
    mProgram->setAttributeBuffer(0, GL_FLOAT, 0, 3, sizeof(GLfloat) * 3);
    mProgram->enableAttributeArray(0) ;
    ...
    mVBOIndex->bind();
    mVBOIndex->allocate(indices, isize);

    initilizeUniformValue();
    mInput.size = vsize/3/sizeof(GLfloat) ;
...
void RenderWindow::renderGL()
{
    ...
    mInput.model = Lmatrix<float>(4,4) ;
    mInput.model.setIdentity() ;
    mInput.view = Lmatrix<float>(ca->getViewMatrix()) ;
    mInput.projection = Lmatrix<float>(ca->getProjectionMatrix()) ;

    bindUniform();

    mVAO->bind();
    glDrawElements(GL_TRIANGLES, mInput.size , GL_UNSIGNED_INT, 0);
    mVAO->release();
...
void RenderWindow::bindUniform()
{
    if (mProgram != nullptr) {
        ...
        mProgram->setUniformValue("model", mInput.model) ;
        mProgram->setUniformValue("view", mInput.view) ;
        mProgram->setUniformValue("projection", mInput.projection) ;
