...
//Window::Window(float* vertices, int vsize,  float* indices, int isize)
Window::Window()
:_window(nullptr) ,
_context(nullptr) ,
_rect(nullptr),
...
{
    createWindow() ;
    ca = new Cam3D(Vec3(3,0,3),Vec3(0,0,1), Vec3(0,0,0)) ;
}
...
void Window::run(float* vertices, int vsize,  int* indices, int isize)
{
    if (!_window && !_context)
        return ;
    ...
    initilizeGL(vertices, vsize, indices, isize);
...
void Window::initilizeGL(float* vertices, int vsize, int* indices, int isize)
{
